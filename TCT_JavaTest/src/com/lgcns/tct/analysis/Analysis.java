...
public class Analysis {	
	...
	public List<String> getProductList(List<SalesInfo> inputData) {
		...
		for(SalesInfo data : inputData) {
			AnalysisInfo info = null;
			if(productMap.containsKey(data.getProduct())) {
		...
		for(String key : productMap.keySet()) {
			...
			double cancelPct;
			double changePct;
			cancelPct =	Math.round((info.getCancleCount()/info.getTotalCount())*100);
			changePct = Math.round((info.getChangeCount()/info.getTotalCount())*100);
			
			if(cancelPct >= 40 && changePct >= 40) {
				productList.add(key);
			}
		}
